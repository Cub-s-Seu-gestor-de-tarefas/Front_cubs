  ...
  const [mobileMenu, setMobileMenu] = useState(false);
    ...
    return (
        <ThemeProvider theme={theme}>
            {/* Sidebar */}
            <Sidebar toggleTheme={toggleTheme} handleMobileToggleMenu={handleMobileToggleMenu} sidebarToggle={sidebarToggle} mobileMenu={mobileMenu} resizeSidebar={resizeSidebar} />
            {/** Chat-Button */}
            <C.ChatButton 
                style={{
                    display: chatInRoom ? 'block' : 'none'
                }}
                viewChat={viewChat} onClick={() => {
                    setViewChat();
                }}
            >
                <svg 
                ...
                <svg xmlns="http://www.w3.org/2000/svg" style={{display: viewChat ? 'initial' : 'none'}} fill="currentColor" className="bi bi-x-lg" viewBox="0 0 16 16">
